...
// Bu ekran iki amaçla açılabilir:
// 1) Bir daireyi satarken alıcı seçmek için (route.params.unitId dolu gelir)
// 2) Sadece alıcı listesini görüntülemek için (unitId boş gelir)
export default function SelectBuyerScreen({ route, navigation }: any) {
  ...
  async function handleSelect(buyer: Buyer) {
    ...
    try {
      ...
      // İki ekran geri git: SelectBuyer -> UnitDetail -> ProjectDetail.
      // UnitDetail'deki veri artık eski olduğu için orada durmuyoruz; ProjectDetail
      // useFocusEffect sayesinde kendini otomatik tazeliyor, güncel durumu orada görüyoruz.
      navigation.pop(2);
    } catch (error: any) {
